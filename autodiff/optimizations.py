...
def bfgs(func, initial_guess,iterations =100):
    ...
    #bfgs for scalar functions
        ...
        #Vector case of BFGS optimization 
            ...
            intermediate_2 = np.dot(np.dot(intermediate1,h0),intermediate1)
...
def gradient_descent(func,initial_guess,iterations = 100,step_size=0.01):
    ...
    if isinstance(initial_guess,Number):
        ...
        #operations are different due to different properties of jacobians for Number and Array

        # e.g. R2 --> R3
        if isinstance(func(initial_guess),tuple):
            x0=initial_guess
            jacobians = []
            s = -Array(func(initial_guess)).jacobian(x0)
            jacobians.append(s)
            ...
            for i in range(iterations):
                ...
                s = -Array(func(x0)).jacobian(x0)

                jacobians.append(s)
            return x0,func(x0),jacobians

        # e.g. R2 --> R1
        elif isinstance(func(initial_guess),Number):
            x0=initial_guess
            jacobians = []
